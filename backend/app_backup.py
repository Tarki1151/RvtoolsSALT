...
def api_dr_analysis():
    ...
    try:
        ...
        # Identify production (on) and replica (off) VMs
        ...
        # Function to extract base VM name
        ...
        # Match replicas with production VMs
        matched_pairs = []
        unmatched_replicas = []
        ...
        for _, replica in replica_candidates.iterrows():
            ...
            # Check if there's a matching production VM (different DC or same base name)
            if base_name in prod_by_base:
                prod = prod_by_base[base_name]
                # Only match if in different datacenter (likely DR scenario)
            ...
            if len(same_name_prod) > 0:
                ...
                # Check if this looks like a replica (has replica naming pattern)
        ...
        for dc in unique_dr_dcs:
            ...
            if len(dc_hosts) > 0:
                ...
                # Calculate required resources for this DR site
                ...
                # Capacity ratio (can DR handle failover?)
        ...
        # Identify critical VMs without replicas (powered on, no matching replica)
        ...
        # Sort by resource usage to find most critical unprotected
        unprotected_vms = unprotected_vms.copy()
        unprotected_vms['ResourceScore'] = (unprotected_vms['CPUs'] * 2 + 
        ...
        return jsonify({
            ...
            'matched_pairs': matched_pairs[:100],  # Limit for performance
